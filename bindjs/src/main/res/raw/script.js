...
function AnimationViewModifier({ args, name }) {
    // Single object arg: { animation, value }
    const opts = args[0] ?? {};
    const animArg = opts.animation ?? null;
    const value = opts.value;
    const stringValue = value != null
        ? String(value)
        : null;

    // Unwrap animation component function to get AST,
    // same pattern as withAnimation().
    let animProps = null;
    if (typeof animArg === 'function'
        && animArg._component) {
        const ast = animArg();
        if (ast) {
            animProps = {
        ...
        && typeof animArg === 'object') {
        animProps = animArg.props ?? animArg;
    }

    // JSON-stringify so AnyDecodable doesn't misinterpret
    // the { type: "spring", ... } dict as a Directive.
    return {
        props: {
            animation: animProps
                ? JSON.stringify(animProps)
                : null,
            value: stringValue
        }
    };
...
function useState(initialValue) {
    ...
    // Get or create hook storage keyed by component path (persists across re-renders)
    let hookKey = this.hookState.path.join('.');
    var hooks = this.hookState.componentHookStore[hookKey] ?? (this.hookState.componentHookStore[hookKey] = []);
    this.hookState.currentComponent.hookStorage = hooks;
...
exports.default = defineComponent({
    ...
    */
    ...
    /**
     * Restores only the environment context (not hookState.path).
     * Used by GeometryReader callbacks which fire asynchronously.
     * @param {*} environmentId
     */
...
// MARK: - Android adapter
// Wraps BindJSRuntime for Android's JavaScriptSandbox which requires
// string return values (customJSONStringify) instead of native objects.
...
Object.assign(this, {
    ...
    callComponentPreview: (args) => customJSONStringify(runtime.callComponentPreview(...args), null, 2),
    getComponentPreviewsWithMetadata: (args) => customJSONStringify(runtime.getComponentPreviewsWithMetadata(...args), null, 2),
    callComponentThumbnail: (componentName, thumbnailOptions, params, children, unwrap, ...args) => customJSONStringify(runtime.callComponentThumbnail(componentName, thumbnailOptions = {}, params, children, unwrap = true, ...args), null, 2),
    ...
    callEventHandler: (handlerId, ...args) => {
        ...
        if (handler) {
            return customJSONStringify(handler(...args))
        }
        return null
    },
    callGeometryReaderComponent: (handlerId, dimensions, environmentId) => {
        ...
        var proxy = {
            ...
            frame: (frameType) => {
                return dimensions[frameType]
            },
            bounds: () => {}
        };
        return callEventHandler(handlerId, proxy);
    },
    callButtonStyleHandler:(handlerId, inner, isPressed) => {
        let label = inner.props.rawValue;
        let labelComponent = runtime.makeComponent(() => {
            return AST.Directive('Text', { rawValue: label });
        });
        const handler = runtime.storedFunctions[handlerId];
        if (handler) {
            let result = handler({label: labelComponent, isPressed: isPressed}, {})();
            return customJSONStringify(result);
        }
        return null;
    },
    ...
    setMcpHost: (enabled) => {
        if (enabled) {
            const send = (name) => (...args) =>
                console.log('__MCP__::' + name + '::' + customJSONStringify(args));
            runtime.mcpHost = {
